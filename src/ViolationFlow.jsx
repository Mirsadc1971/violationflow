...
const SUPA_URL = SB;
const SUPA_KEY = SK;
const SH = {"Content-Type":"application/json","x-api-key":"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","anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true",apikey:SK,Authorization:`Bearer ${SK}`,Prefer:"return=representation"};
...
/* ─────────────────────────────────────────────────────────────────────────────
   SUPABASE AUTH
───────────────────────────────────────────────────────────────────────────── */
const AH = {"Content-Type":"application/json","x-api-key":"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","anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true",apikey:SK};
...
Never give legal advice. Recommend consulting an attorney for legal questions.`;
  ...
  const send=async()=>{
    ...
    try{
      ...
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":"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","anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:600,system:SYS,messages:history})});
...
function ResidentForm({onBack}) {
  ...
  const submitReg=async()=>{
    ...
    setSaving(true);
    try{
      const res=await fetch(`${SUPA_URL}/auth/v1/signup`,{method:"POST",headers:{"Content-Type":"application/json","x-api-key":"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","anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true","apikey":SUPA_KEY},body:JSON.stringify({email:reg.email,password:reg.password,data:{owner_name:reg.owner_name,assoc_name:reg.assoc_name,address:reg.address,unit:reg.unit,city:reg.city,state:reg.state,zip:reg.zip,phone:reg.phone}})});
      const data=await res.json();
...
function RulesTab({assocs,rules,companyId,onSave}) {
  ...
  const handlePDF=async e=>{
    ...
    try{
      ...
      const resp=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":"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","anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({
...
function Dashboard({onBack,session,onSignOut}) {
  ...
  const load=async()=>{setLoading(true);const[r,c,a,ru,l]=await Promise.all([db(`violation_reports?select=*,rules(*),associations(*)&order=created_at.desc${cid?"&company_id=eq."+cid:""}`),db(`violation_cases?select=*,violation_reports(*),rules(*),associations(*)&order=created_at.desc${cid?"&company_id=eq."+cid:""}`),db(`associations?select=*&order=name.asc${cid?"&company_id=eq."+cid:""}`),db(`rules?select=*&order=rule_title.asc${cid?"&company_id=eq."+cid:""}`),db("leads?select=*&order=created_at.desc&limit=50").catch(()=>[])]);setReports(Array.isArray(r)?r:[]);setCases(Array.isArray(c)?c:[]);setAssocs(Array.isArray(a)?a:[]);setRules(Array.isArray(ru)?ru:[]);setLeads(Array.isArray(l)?l:[]);setLoading(false);};
  useEffect(()=>{load();},[]);
