...
const SUPA_URL = SB;
const SUPA_KEY = SK;
const SH = {"Content-Type":"application/json","x-api-key":"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","anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true",apikey:SK,Authorization:`Bearer ${SK}`,Prefer:"return=representation"};
...
/* ─────────────────────────────────────────────────────────────────────────────
   SUPABASE AUTH
───────────────────────────────────────────────────────────────────────────── */
const AH = {"Content-Type":"application/json","x-api-key":"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","anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true",apikey:SK};
...
Never give legal advice. Recommend consulting an attorney for legal questions.`;
  ...
  const send=async()=>{
    ...
    try{
      ...
      const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":"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","anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:600,system:SYS,messages:history})});
...
function ResidentForm({onBack}) {
  ...
  const submitReg=async()=>{
    ...
    setSaving(true);
    try{
      const res=await fetch(`${SUPA_URL}/auth/v1/signup`,{method:"POST",headers:{"Content-Type":"application/json","x-api-key":"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","anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true","apikey":SUPA_KEY},body:JSON.stringify({email:reg.email,password:reg.password,data:{owner_name:reg.owner_name,assoc_name:reg.assoc_name,address:reg.address,unit:reg.unit,city:reg.city,state:reg.state,zip:reg.zip,phone:reg.phone}})});
      const data=await res.json();
...
function RulesTab({assocs,rules,companyId,onSave}) {
  ...
  const handlePDF=async e=>{
    ...
    try{
      ...
      const resp=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":"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","anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},body:JSON.stringify({
