...
function Dashboard({onBack,session,onSignOut}) {
  ...
  const load=async()=>{setLoading(true);const[r,c,a,ru,l]=await Promise.all([db(`violation_reports?select=*,rules(*),associations(*)&order=created_at.desc${cid?"&company_id=eq."+cid:""}`),db(`violation_cases?select=*,violation_reports(*),rules(*),associations(*)&order=created_at.desc${cid?"&company_id=eq."+cid:""}`),db(`associations?select=*&order=name.asc${cid?"&company_id=eq."+cid:""}`),db(`rules?select=*&order=rule_title.asc${cid?"&or=(company_id.eq."+cid+",company_id.is.null)":""}`),db("leads?select=*&order=created_at.desc&limit=50").catch(()=>[])]);setReports(Array.isArray(r)?r:[]);setCases(Array.isArray(c)?c:[]);setAssocs(Array.isArray(a)?a:[]);setRules(Array.isArray(ru)?ru:[]);setLeads(Array.isArray(l)?l:[]);setLoading(false);};
  useEffect(()=>{load();},[]);
